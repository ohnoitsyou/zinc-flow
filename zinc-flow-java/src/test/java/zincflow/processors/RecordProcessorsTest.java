...
final class RecordProcessorsTest {
    ...
    void convertJsonArrayToRecords() {
        var ff = FlowFile.Companion.create("[{\"id\":1,\"name\":\"a\"},{\"id\":2,\"name\":\"b\"}]".getBytes(), Map.of());
        ...
        assertEquals(1, ((Number) rc.getRecords().get(0).get("id")).intValue());
    ...
    void convertJsonSingleObjectWrappedInList() {
        var ff = FlowFile.Companion.create("{\"id\":42}".getBytes(), Map.of());
        var out = (ProcessorResult.Single) new ConvertJSONToRecord().process(ff);
        var rc = (RecordContent) out.flowFile.getContent();
    ...
    void convertJsonMalformedRoutesToFailure() {
        var ff = FlowFile.Companion.create("{bad".getBytes(), Map.of());
    ...
    void convertRecordsToJsonArray() throws Exception {
        ...
        var processed = new ConvertRecordToJSON().process(ff);
        if (!(processed instanceof ProcessorResult.Single out) || !(out.flowFile.getContent() instanceof RawContent raw)) {
        ...
        if (!(new ConvertJSONToRecord().process(FlowFile.Companion.create(raw.getBytes(), Map.of()))
                instanceof ProcessorResult.Single roundTrip)
                || !(roundTrip.flowFile.getContent() instanceof RecordContent decoded)) {
            fail("ConvertJSONToRecord should produce Single(RecordContent)");
            return;
        }
        assertEquals(2, decoded.getRecords().size());
    }
    ...
    void convertRecordToJsonSingleObjectEmitsBareObject() {
        ...
        var raw = (RawContent) out.flowFile.getContent();
        String json = new String(raw.getBytes());
    ...
    void extractFieldHoistsValueToAttribute() {
        ...
        assertEquals("acme", out.flowFile.getAttributes().get("tenant.id"));
    }
    ...
    void extractFieldSupportsMultipleFieldPairs() {
        ...
        assertEquals("acme", out.flowFile.getAttributes().get("tenant.id"));
        assertEquals("high", out.flowFile.getAttributes().get("priority.level"));
    }
    ...
    void extractFieldSupportsDottedPath() {
        ...
        assertEquals("7", out.flowFile.getAttributes().get("user.id"));
    }
    ...
    void extractFieldRecordIndexSelectsSpecificRecord() {
        ...
        assertEquals("third", out.flowFile.getAttributes().get("t"));
    }
    ...
    void extractFieldMissingPathSilentlySkipped() {
        ...
        assertFalse(out.flowFile.getAttributes().containsKey("out"));
    }
